defmodule NotesclubWeb.PageController do
  ...
  def index(conn, _params) do
    ...
    render(conn, "index.html", notebooks: notebooks, notebooks_count: notebooks_count, page: :random, filter: nil)
  end

  def all(conn, _params) do
    notebooks = Notebooks.list_notebooks_desc()
    notebooks_count = Notebooks.count()
    render(conn, "index.html", notebooks: notebooks, notebooks_count: notebooks_count, page: :all, filter: nil)
  end

  def last_week(conn, _params) do
    notebooks = Notebooks.list_notebooks_since(7)
    notebooks_count = Notebooks.count()
    render(conn, "index.html", notebooks: notebooks, notebooks_count: notebooks_count, page: :last_week, filter: nil)
  end
  ...
  def repo(conn, %{"repo" => repo, "author" => author}) do
    ...
    notebooks_count = Notebooks.count()
    render(conn, "index.html", notebooks: notebooks, notebooks_count: notebooks_count, page: :repo, filter: "#{author}/#{repo}")
  end
end
