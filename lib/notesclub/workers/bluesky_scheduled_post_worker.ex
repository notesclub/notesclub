defmodule Notesclub.Workers.BlueskyScheduledPostWorker do
  @moduledoc """
  Worker to post scheduled messages to Bluesky using the stored access token
  Scheduled via cron to run every 8 hours
  """
  use Oban.Worker,
    queue: :default,
    max_attempts: 3

  require Logger

  alias Notesclub.Accounts.User
  alias Notesclub.Bluesky
  alias Notesclub.Notebooks
  alias Notesclub.Notebooks.Notebook
  alias Notesclub.Notebooks.Paths
  alias Notesclub.PublishLogs

  @platform "bluesky"

  @impl Oban.Worker
  def perform(%Oban.Job{}) do
    case Notebooks.get_non_published_most_starred_notebook(@platform) do
      nil ->
        case Notebooks.get_non_published_highest_ai_rated_notebook(@platform) do
          nil -> {:ok, "No non-shared notebook found"}
          notebook -> share(notebook)
        end

      notebook ->
        share(notebook)
    end
  end

  defp share(notebook) do
    path = Paths.url_to_path(notebook)
    message = get_message(notebook, path)

    case Bluesky.post(message) do
      {:ok, _response} ->
        create_publish_log(notebook)

      {:error, reason} ->
        {:error, "Failed to post to Bluesky: #{inspect(reason)}"}
    end
  end

  defp get_message(%Notebook{user: %User{name: nil, username: nil}} = notebook, path) do
    "#{notebook.title} https://notes.club#{path} #ElixirLang"
  end

  defp get_message(%Notebook{user: %User{name: nil, username: username}} = notebook, path) do
    "#{notebook.title} by #{username} https://notes.club#{path} #ElixirLang"
  end

  defp get_message(%Notebook{} = notebook, path) do
    "#{notebook.title} by #{notebook.user.name} https://notes.club#{path} #ElixirLang"
  end

  defp create_publish_log(notebook) do
    case PublishLogs.create_publish_log(%{
           platform: @platform,
           notebook_id: notebook.id,
           user_id: notebook.user_id
         }) do
      {:ok, _} ->
        :ok

      {:error, reason} ->
        Logger.error(
          "Failed to create_publish_log after posting to Bluesky (be aware: we might publish repeated notebooks): #{inspect(reason)}"
        )

        {:error, "Failed to create publish log: #{inspect(reason)}"}
    end
  end
end
