defmodule Notesclub.Notebooks.Rater.API do
  ...
  defp try_structured_request(content, api_key) do
    ...
    case Req.post(
      ...
      {:ok, %{status: 200, body: body}} ->
        Logger.info("Structured request response: #{inspect(body)}")
    ...
    %{
      ...
      messages: [
        %{
          ...
          1. **Elixir Code Quality**: Presence of well-written Elixir code cells
          2. **Educational Value**: How much an Elixir developer would learn
          3. **Practical Applications**: Real-world use cases and examples
          4. **Code Complexity**: Sophistication of the Elixir concepts demonstrated
          5. **Package Usage**: Use of interesting Elixir/Erlang libraries
          6. **Content Length**: Depth and comprehensiveness of the material
          7. **Documentation Quality**: Clear explanations and context
          8. **Innovation**: Novel approaches or cutting-edge techniques

          Heavily penalize notebooks that:
          - Have no Elixir code cells
          - Are very short or superficial
          - Focus primarily on other programming languages
          - Contain only basic "Hello World" examples

          Reward notebooks that:
          - Demonstrate advanced Elixir/OTP concepts
  ...
  defp parse_rating_response(%{"choices" => [%{"message" => %{"content" => content}} | _]}) do
    Logger.info("Received content: #{inspect(content)}")

    # Handle empty or nil content
    case content do
      nil ->
        Logger.error("Received nil content from API")
        {:error, :nil_content}
      ...
      other ->
        Logger.error("Received non-string content: #{inspect(other)}")
        {:error, :invalid_content_type}
    end
  end

  # Handle alternative response structures that OpenRouter might send
  defp parse_rating_response(%{"choices" => [%{"message" => message} | _]}) when is_map(message) do
    Logger.info("Alternative message structure: #{inspect(message)}")

    # Check if there's a tool_calls field instead of content
    cond do
      Map.has_key?(message, "tool_calls") ->
        Logger.info("Response has tool_calls instead of content")
        {:error, :tool_calls_not_supported}

      Map.has_key?(message, "function_call") ->
        Logger.info("Response has function_call instead of content")
        {:error, :function_call_not_supported}

      true ->
        Logger.error("Message has no recognizable content field: #{inspect(Map.keys(message))}")
        {:error, :no_content_field}
    end
  end

  defp parse_rating_response(response) do
    ...
    case content do
      ...
      content when is_binary(content) ->
        ...
        case Jason.decode(cleaned_content) do
          ...
          {:ok, invalid_response} ->
            Logger.error("Invalid fallback rating response structure: #{inspect(invalid_response)}")
            {:error, :invalid_response}
